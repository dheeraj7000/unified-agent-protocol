...
from uap import CapabilityCard, UAPRuntime, CapabilityRegistry, PolicyEngine, ContextManager
...
from uap.errors import PolicyDeniedError, ApprovalRequiredError
from uap.models import TaskEnvelope, Actor, Intent, Policy
...
def build_runtime() -> UAPRuntime:
    ...
    def list_customers(input_value, envelope):
        return [
            {"id": "C001", "name": "Acme Corp", "balance": 12500, "status": "active", "internal_notes": "VIP"},
            {"id": "C002", "name": "Globex Inc", "balance": 8400, "status": "active", "internal_notes": "Standard"},
            {"id": "C003", "name": "Initech LLC", "balance": 0, "status": "churned", "internal_notes": "Do not contact"},
            {"id": "C004", "name": "Umbrella Co", "balance": 45000, "status": "active", "internal_notes": "Enterprise"},
            {"id": "C005", "name": "Stark Ind", "balance": 92000, "status": "active", "internal_notes": "Strategic"},
        ]

    def list_invoices(input_value, envelope):
        return [
            {"invoice_id": "INV-1001", "customer": "Acme Corp", "amount": 1200, "due": "2026-05-01", "status": "overdue"},
            {"invoice_id": "INV-1002", "customer": "Globex Inc", "amount": 840, "due": "2026-05-12", "status": "overdue"},
            {"invoice_id": "INV-1003", "customer": "Stark Ind", "amount": 5600, "due": "2026-06-01", "status": "paid"},
        ]

    def draft_email(input_value, envelope):
        prev = input_value.get("previous_results", {})
        return {"drafts": [{"to": "billing@example.com", "subject": "Reminder", "body": "Overdue invoices attached."}]}
    ...
    capabilities = [
        (CapabilityCard("customer.list", "List all customers", {"type": "object"}, {"type": "array"},
                        risk="low", permissions=["customer.read"], tags=["customer", "crm", "read"]), list_customers),
        (CapabilityCard("invoice.list_overdue", "Find overdue invoices", {"type": "object"}, {"type": "array"},
                        risk="low", permissions=["invoice.read"], tags=["invoice", "finance", "read"]), list_invoices),
        (CapabilityCard("email.draft", "Draft emails", {"type": "object"}, {"type": "object"},
                        risk="medium", permissions=["email.draft"], tags=["email", "communication"]), draft_email),
        (CapabilityCard("email.send", "Send emails to recipients", {"type": "object"}, {"type": "object"},
                        risk="high", requires_approval=True, permissions=["email.send"], tags=["email", "communication"]), send_email),
        (CapabilityCard("analytics.report", "Generate analytics report", {"type": "object"}, {"type": "object"},
                        risk="low", permissions=["analytics.read"], tags=["analytics", "reporting"]), slow_analytics),
        (CapabilityCard("analytics.forecast", "Generate revenue forecast", {"type": "object"}, {"type": "object"},
                        risk="low", permissions=["analytics.read"], tags=["analytics", "reporting"]), slow_forecast),
        (CapabilityCard("customer.delete", "Delete a customer record permanently", {"type": "object"}, {"type": "object"},
                        risk="critical", requires_approval=True, permissions=["customer.delete"], tags=["customer", "crm", "destructive"]), delete_customer),
        (CapabilityCard("db.query", "Run a read-only database query", {"type": "object"}, {"type": "object"},
                        risk="low", permissions=["db.read"], tags=["database", "read"]), read_only_query),
    ]
...
async def test_1_envelope_validation():
    ...
    report("Rejects missing actor/intent", result["status"] == "failed" and "INVALID_ENVELOPE" in result["error"]["code"])

    # 1b. Invalid actor type
    result = await runtime.invoke({"uap": "1.0", "type": "task.invoke", "task_id": "tsk_v2", "actor": "not_an_object", "intent": {"goal": "test"}})
    report("Rejects non-object actor", result["status"] == "failed" and "INVALID_ACTOR" in result["error"]["code"])

    # 1c. Missing agent_id in actor
    result = await runtime.invoke({"uap": "1.0", "type": "task.invoke", "task_id": "tsk_v3", "actor": {}, "intent": {"goal": "test"}})
    report("Rejects actor without agent_id", result["status"] == "failed" and "INVALID_ACTOR" in result["error"]["code"])

    # 1d. Invalid constraints type
    result = await runtime.invoke({"uap": "1.0", "type": "task.invoke", "task_id": "tsk_v4",
                                    "actor": {"agent_id": "a"}, "intent": {"goal": "test"},
                                    "constraints": {"latency_ms": "not_an_int"}})
    report("Rejects non-integer latency_ms", result["status"] == "failed" and "INVALID_CONSTRAINTS" in result["error"]["code"])

    # 1e. Invalid execution graph
    result = await runtime.invoke({"uap": "1.0", "type": "task.invoke", "task_id": "tsk_v5",
                                    "actor": {"agent_id": "a"}, "intent": {"goal": "test"},
                                    "execution": {"graph": {"nodes": [{"id": 123, "capability": "x"}]}}})
    report("Rejects graph node with non-string id", result["status"] == "failed" and "INVALID_EXECUTION_GRAPH" in result["error"]["code"])

    # 1f. Valid payload passes validation
    result = await runtime.invoke({"uap": "1.0", "type": "task.invoke", "task_id": "tsk_v6",
                                    "actor": {"agent_id": "agent_1"}, "intent": {"goal": "list customers"},
...
async def test_2_policy_enforcement():
    ...
    result = await runtime.invoke({
        "uap": "1.0", "type": "task.invoke", "task_id": "tsk_p1",
        "actor": {"agent_id": "agent_1"}, "intent": {"goal": "draft email"},
        "policy": {"allowed_tools": ["email.draft"]},
        "execution": {"graph": {"nodes": [{"id": "n1", "capability": "email.send"}]}}
    })
    report("Blocks tool not in allowed_tools", result["status"] == "failed" and result["error"]["code"] == "TOOL_NOT_ALLOWED")
    ...
    result = await runtime.invoke({
        "uap": "1.0", "type": "task.invoke", "task_id": "tsk_p2",
        "actor": {"agent_id": "agent_1"}, "intent": {"goal": "delete customer"},
        "policy": {"max_risk": "medium"},
        "execution": {"graph": {"nodes": [{"id": "n1", "capability": "customer.delete"}]}}
    })
    report("Blocks capability exceeding max_risk", result["status"] == "failed" and result["error"]["code"] == "RISK_EXCEEDS_POLICY")
    alts = result.get("error", {}).get("alternative_capabilities", [])
    report("Suggests lower-risk alternatives with shared tags", "customer.list" in alts, f"alternatives={alts}")

    # 2c. Requires approval gate
    result = await runtime.invoke({
        "uap": "1.0", "type": "task.invoke", "task_id": "tsk_p3",
        "actor": {"agent_id": "agent_1"}, "intent": {"goal": "send email"},
        "execution": {"graph": {"nodes": [{"id": "n1", "capability": "email.send"}]}}
    })
    report("Triggers approval gate for requires_approval=True", result["status"] == "waiting_for_approval")
...
async def test_3_parallel_dag_execution():
    ...
    result = await runtime.invoke({
        "uap": "1.0", "type": "task.invoke", "task_id": "tsk_dag",
        "actor": {"agent_id": "agent_1"}, "intent": {"goal": "analytics"},
        "execution": {
            "parallelism": 4,
            "graph": {
                "nodes": [
                    ...
                    {"id": "query", "capability": "db.query", "depends_on": ["report", "forecast"]},
                ]
            }
        }
    })
    ...
    report(f"Parallel execution faster than serial ({elapsed:.0f}ms < 150ms)", elapsed < 150,
           f"elapsed={elapsed:.0f}ms")

    # Verify dependency was respected — query should have previous_results from report+forecast
    report("Dependency node received upstream results",
    ...
    report("Provenance recorded for all 3 capabilities",
           "analytics.report" in prov_caps and "analytics.forecast" in prov_caps and "db.query" in prov_caps)
...
async def test_4_context_compaction():
    ...
    result = await runtime.invoke({
        "uap": "1.0", "type": "task.invoke", "task_id": "tsk_ctx1",
        "actor": {"agent_id": "agent_1"}, "intent": {"goal": "list customers"},
        "context_request": {"fields": ["name", "status"], "max_items": 20},
        "policy": {"allowed_tools": ["customer.list"]},
    })
    first_item = result["result"]["nodes"]["n1"][0]
    report("Field mask strips unrequested fields", "internal_notes" not in first_item and "balance" not in first_item)
    report("Field mask preserves requested fields", first_item.get("name") == "Acme Corp" and first_item.get("status") == "active")

    # 4b. Item limit — only return 2 of 5 customers
    result = await runtime.invoke({
        "uap": "1.0", "type": "task.invoke", "task_id": "tsk_ctx2",
        "actor": {"agent_id": "agent_1"}, "intent": {"goal": "list customers"},
    ...
    report("Truncation marker shows remaining count", items[-1].get("_truncated") is True and items[-1].get("remaining_items") == 3)

    # 4c. Token budget — very tight budget forces summarization
    result = await runtime.invoke({
        "uap": "1.0", "type": "task.invoke", "task_id": "tsk_ctx3",
        "actor": {"agent_id": "agent_1"}, "intent": {"goal": "list customers"},
        "constraints": {"max_context_tokens": 10},
        "policy": {"allowed_tools": ["customer.list"]},
    })
    output = result["result"]["nodes"]["n1"]
    report("Token budget triggers truncation/summary", isinstance(output, dict) and output.get("_truncated") is True)
...
async def test_5_provenance_and_events():
    ...
    result = await runtime.invoke({
        "uap": "1.0", "type": "task.invoke", "task_id": "tsk_prov",
    ...
    expected = ["task.accepted", "plan.proposed", "tool.started", "tool.completed", "partial.result", "task.completed"]
    report("Full lifecycle events emitted", all(t in event_types for t in expected),
           f"got={event_types}")

    # CloudEvents conversion
    ce = uap_event_to_cloudevent(events[0])
    report("CloudEvents adapter produces spec 1.0", ce["specversion"] == "1.0" and ce["source"] == "/uap/tasks/tsk_prov")
...
async def test_6_adapter_import():
                ...
                "post": {
                    ...
                    "requestBody": {
                        ...
                        "content": {"application/json": {"schema": {
                            "type": "object",
                            "properties": {"amount": {"type": "number"}, "customer_id": {"type": "string"}},
    ...
    card = cards[0]
    report("OpenAPI card has parsed input properties", len(card.input_schema.get("properties", {})) > 0)
    ...
    report("MCP card ID includes server namespace", mcp_cards[0].capability_id == "browser.web_search")
    report("MCP card preserves input schema", "query" in mcp_cards[0].input_schema.get("properties", {}))
...
async def test_7_structured_error_recovery():
    ...
    result = await runtime.invoke({
        "uap": "1.0", "type": "task.invoke", "task_id": "tsk_err1",
        "actor": {"agent_id": "agent_1"}, "intent": {"goal": "nonexistent"},
    ...
    report("Error events emitted to event bus", "error.terminal" in events or "error.recoverable" in events)
...
async def test_8_end_to_end_realistic():
    ...
    if isinstance(invoices, list) and len(invoices) > 0:
        report("Context contract stripped unrequested fields", "internal_notes" not in str(invoices[0]))
    else:
